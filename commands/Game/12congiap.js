export default {
  ...
  aliases: [],
  execute: async ({ event }) => {
    return kaguya.reply("==== 12 con giáp trong dân gian ====" + "\n1. Tý 🐁" + "\n2. Sửu 🐃" + "\n3. Dần 🐅" + "\n4. Mão 🐈" + "\n5. Thìn 🐉" + "\n6. Tỵ 🐍" + "\n7. Ngọ 🦓" + "\n8. Mùi 🐐" + "\n9. Thân 🐒" + "\n10. Dậu 🐓" + "\n11. Tuất 🐕" + "\n12. Hợi 🐖" + "\n\n🌹Reply tin nhắn theo số để xem thêm về từng con giáp ❤", (err, info) => {
  ...
  onReply: async ({ event, reply }) => {
    if (reply.type === "pick") {
      const choices = ["Tý ( Chuột 🐁 )", "Sửu ( Tru 🐃 )", "Dần ( Hổ 🐅 )", "Mão ( Mèo 🐈 )", "Thìn ( Rồng 🐉 )", "Tỵ ( Rắn 🐍 )", "Ngọ ( Ngựa 🦓 )", "Mùi ( Dê 🐐 )", "Thân ( Khỉ 🐒 )", "Dậu ( Gà 🐓 )", "Tuất ( Chó 🐕 )", "Hợi ( Lợn 🐖 )"];
      const choiceIndex = parseInt(event.body);
      if (isNaN(choiceIndex) || choiceIndex < 1 || choiceIndex > 12) {
        return kaguya.reply("Vui lòng nhập 1 con số từ 1 đến 12.");
      }
      const choiceDescription = choices[choiceIndex - 1];
      kaguya.reply(choiceIndex + ". " + choiceDescription + "\n\n" + getDescription(choiceIndex));
    }
  },
};
